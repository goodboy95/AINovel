...
- **后续步骤**：完成故事设定后，可直接进入“大纲工作台”进行规划，或在“故事创作”页面开始写作。
...
目标：在规划好大纲后，逐个“场景”生成对应的正文，并可编辑与保存。

入口与界面：
- 进入“工作台”→“故事创作”页签。
- **左侧**：新增了一个“大纲选择栏”，列出了当前故事下的所有可用大纲。
- **右侧**：保留了原有的大纲结构树和正文编辑器。

操作步骤：
1) 选择一个大纲
   - 首先在左侧的“大纲选择栏”中，点击你想要进行创作的大纲。

2) 选择一个场景
   - 右侧的大纲树会加载所选大纲的内容。从树状结构中点击一个“场景”。

3) 生成正文
   - 点击“✨ 生成内容”，系统会综合“故事设定/角色/大纲上下文/当前场景梗概”等信息进行创作。
   - 生成完成后，正文自动填入编辑器。

4) 编辑与保存
   - 直接在编辑器内修改任何内容。
   - 点击“保存”将内容提交（建议随写随存，避免丢失）。

5) 预计字数
   - 编辑器右下方会显示“预期字数”，供你把控篇幅。

小贴士：
- 如对创作风格不满意，可先回到“大纲工作台”改进该场景的梗概与人物状态，再回来重新“生成内容”。
- 多次点击“生成内容”会以最新一次为准（版本选择功能将按规划逐步完善）。
...
安全说明：
- 你的 API Key 会在服务端以加密方式保存，并且接口不会回传完整密钥给前端页面；界面上保存后也不会再次明文显示。
- 传输过程中使用安全协议（取决于你的部署环境配置）；请勿将密钥分享给他人。
...
- 第三步：在“故事创作”中重新“生成内容”。必要时，调整“每节大概字数”，以便 AI 有更充分的发挥空间。

Q3：如何导出我的作品？📤  
A：当前版本尚未提供一键导出功能。临时解决方案：
- 在“故事创作”中选中正文，复制到你常用的写作软件（如 Word、WPS、Notion、Obsidian）；
...
Q6：我的内容会丢失吗？💾  
A：请在“故事创作”中养成“写完一段就点击保存”的习惯。网络抖动、浏览器意外关闭或 Token 过期都可能导致未保存的内容丢失。重要内容建议同时备份到本地文档。
