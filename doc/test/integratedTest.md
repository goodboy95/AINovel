...
- **认证链路**：注册（人机验证→验证码）→登录→`/api/v1/user/profile`；预期获得 JWT 并能访问受保护路由。
...
- **部署验证**：执行 `deploy/build.sh` + `build_prod.sh --init` 后 docker 容器健康，浏览器通过 `https://ainovel.aienie.com/` 正常访问，后端 `https://ainovel.aienie.com/api/*` 正常响应。
