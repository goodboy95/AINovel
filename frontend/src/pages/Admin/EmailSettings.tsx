    ...
    <div className="space-y-6">
      ...
      <Card className="bg-zinc-900 border-zinc-800 text-zinc-100">
        ...
        <CardContent className="space-y-2 text-sm text-zinc-300">
          <div>Host: <span className="font-mono text-zinc-100">{smtp?.host || "-"}</span></div>
          <div>Port: <span className="font-mono text-zinc-100">{smtp?.port || "-"}</span></div>
          <div>Username: <span className="font-mono text-zinc-100">{smtp?.username || "-"}</span></div>
          <div>Password: <span className="font-mono text-zinc-100">{smtp?.passwordIsSet ? "已配置" : "未配置"}</span></div>
          <div className="pt-3 flex gap-2">
            <Input
              value={testEmail}
              onChange={(e) => setTestEmail(e.target.value)}
              placeholder="输入收件邮箱发送测试邮件"
